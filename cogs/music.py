...
class Music(commands.Cog, name="음악"):
	...
	@commands.command(name="재생", help="!추가 명령어를 통해 플레이리스트에 저장된 노래를 틀어드려요!", usage="!재생")
	...
	async def skip(self, ctx):
		...
		if voice and voice.is_playing(): # 봇이 음성 채널에서 재생 중이면
			voice.source.cleanup() # 재생을 멈춤
		else:
			await ctx.send("플레이리스트가 비어있습니다. !추가 명령어를 통해 노래를 추가해주세요.")


	@commands.command(name="플레이리스트", help="플레이리스트를 확인합니다!", usage="!플레이리스트")
